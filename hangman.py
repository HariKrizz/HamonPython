...
def game_status(secret_word,guesses,remaining_turns):
...
def main():
    ...
    guesses = []
    while True:
        status = game_status(secret_word, guesses, remaining_turns)
        print(status)
        guess = input("Enter a letter ").strip()
        remaining_turns, repeat, finished = play_round(secret_word, guesses, guess, remaining_turns)
        if finished:
            print("You found the secret word", secret_word)
            break
